...
export const ENDPOINTS = [
  ...
  },
  {
    method: 'GET',
    path: '/random',
    description: 'Returns a full condemnation profile for a randomly selected cheese — a different cheese on every call. Pass ?today_only=true for deterministic daily seeding, or ?tier=<name> to restrict selection to a specific severity tier. Both parameters combine freely.',
    parameters: [
      {
        name: 'today_only',
        location: 'query',
      ...
      },
      {
        ...
        type: 'string',
        required: false,
        description: 'Restrict selection to a specific severity tier. Valid values: catastrophic | revolting | condemned. Returns 400 for unknown tier names. Combine with today_only=true for a deterministic daily worst-of-tier.',
      },
    ],
    example_request: 'GET /random?tier=catastrophic',
    example_response: {
      cheese: 'Casu Martzu',
      score: 0.5,
      severity_tier: 'catastrophic',
      verdict: 'GUILTY',
      worst_quality: 'contains live maggots — an intended feature, not a defect',
      smell_description: 'advanced decomposition with undertones of ammonia',
      texture_offense: 'soft, runny, and actively moving',
      cultural_damage_score: 9,
      found_at: 'Sardinia, where it remains consumed despite being illegal to sell in the EU',
      recommended_alternative: 'anything that does not contain live insects',
      closing_statement: 'Casu Martzu is what cheese becomes when you follow its awful logic without regulatory constraint.',
      etymology: { does_this_help: false, '...': '...' },
      pairings: [{ cheese: 'Blue Cheese', score: 0.8, verdict: 'CATASTROPHIC', note: '...' }],
      roast: '...',
      today_only: false,
      tier: 'catastrophic',
