...
const damageCategoryMap: Record<string, string> = {
  brie: 'mold-adjacent hospitality',
  parmesan: 'gym locker garnish',
  cheddar: 'ubiquitous dairy offense',
  gouda: 'gateway dairy damage',
  'gruyère': 'communal melted suffering',
  halloumi: 'squeaking protest food',
  mozzarella: 'rubbery pizza infrastructure',
  'cottage cheese': 'lumpy wet regret',
  ricotta: 'soft dairy deception',
  feta: 'brined Mediterranean harm',
}
  ...
  res.json({
    documented_cheeses: Object.keys(pairingsData)
      ...
      .map(k => ({
        ...
        damage_category: damageCategoryMap[k] ?? 'unclassified dairy offense',
        endpoint: `/pair/${pairingsData[k].cheese}`,
      })),
