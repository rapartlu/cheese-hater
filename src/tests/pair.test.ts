...
describe('GET /pair — listing all documented cheeses', () => {
  ...
  it('each documented_cheese entry has a damage_category string', async () => {
    const res = await request(app).get('/pair')
    for (const entry of res.body.documented_cheeses) {
      expect(entry).toHaveProperty('damage_category')
      expect(typeof entry.damage_category).toBe('string')
      expect(entry.damage_category.length).toBeGreaterThan(0)
    }
  })

  it('each documented_cheese entry has an endpoint field matching /pair/:cheese', async () => {
    const res = await request(app).get('/pair')
    for (const entry of res.body.documented_cheeses) {
      expect(entry).toHaveProperty('endpoint')
      expect(typeof entry.endpoint).toBe('string')
      expect(entry.endpoint).toMatch(/^\/pair\//)
    }
  })
